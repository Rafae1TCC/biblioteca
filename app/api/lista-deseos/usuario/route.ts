...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`[ListaDeseosAPI] Obteniendo lista de deseos para ${session.user.email}`)

    // Obtener el ID del usuario
    const userResult = await executeQuery("SELECT id FROM usuarios WHERE email = $1", [session.user.email])

    if (userResult.length === 0) {
      console.log(`[ListaDeseosAPI] Usuario no encontrado para ${session.user.email}`)
      return NextResponse.json({ error: "Usuario no encontrado" }, { status: 404 })
    }

    const userId = userResult[0].id
    console.log(`[ListaDeseosAPI] ID de usuario encontrado: ${userId}`)
    ...
    console.log(`[ListaDeseosAPI] Se encontraron ${wishlistResult.length} libros en la lista de deseos`)

    return NextResponse.json({ wishlist: wishlistResult })
  } catch (error) {
    console.error("[ListaDeseosAPI] Error al obtener la lista de deseos:", error)
